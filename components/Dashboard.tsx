  ...
  if (!mainStatus) {
    return (
      <div className="text-center p-8">
        <h2 className="text-sm text-gray-600">No status found.</h2>
        <button onClick={onReset} className="mt-2 text-chase-blue text-sm underline">Back</button>
      </div>
    );
  }
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300 pb-4">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-3">
        ...
        <div className="space-y-1 text-xs">
            <MetaRow label="Product" value={`${mainStatus.productCode} (${mainStatus.subProductCode})`} />
            ...
            <MetaRow label="Created" value={formatDate(data.applicationCreateTimestamp)} />
        </div>
      </div>
...
function formatDate(isoString?: string) {
    ...
    return new Date(isoString).toLocaleDateString(undefined, { month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit' });
}
